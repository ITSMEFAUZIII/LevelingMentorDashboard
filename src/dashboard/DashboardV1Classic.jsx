// src/dashboard/DashboardV1Classic.jsx
...
const ProgressBar = ({value})=>(
  <div className="relative h-3 w-full overflow-hidden rounded-full bg-neutral-700">
...
const RippleBtn=({children,onClick,className=""})=>(
  <button onClick={onClick} className={"rounded-2xl bg-neutral-800 text-white px-4 py-2 hover:opacity-90 "+className}>{children}</button>
);

export default function DashboardV1Classic({ activeUser }) {
  const {
  ...
  return (
    <div className="min-h-screen w-full relative overflow-hidden text-neutral-100">
      <SpaceScene/>
      ...
      <div className="mx-auto max-w-6xl p-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs bg-amber-500/20 text-amber-300">
                  🔥 x{streak} Streak
                </span>
              </div>
              <p className="text-neutral-300">
                Track: <span className="font-medium">{state.track}</span> — XP 7d: <span className="font-medium">{weeklyXP}</span>
              </p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            <button onClick={()=>setOpenStress(true)} className="rounded-2xl border border-neutral-700 px-4 py-2 text-sm bg-neutral-800">Kesehatan & Waktu</button>
            <button onClick={()=>{localStorage.removeItem(`mentor-dashboard-pro-v9:${activeUser}`); location.reload();}} className="rounded-2xl border border-neutral-700 px-4 py-2 text-sm bg-neutral-800">Reset Local</button>
          </div>
        </header>

        {/* XP & Level */}
        <section className="mb-6 rounded-3xl p-5 bg-neutral-900/90 border border-neutral-700">
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-lg font-semibold">XP & Level</h2>
            {xpMultiplier>1 && <div className="rounded-full bg-emerald-500/15 text-emerald-300 px-3 py-1 text-xs">🎉 Double XP Day!</div>}
          </div>
          <div className="flex items-center gap-3">
            <div className="flex-1"><ProgressBar value={progress}/></div>
            <div className="text-sm tabular-nums w-24 text-right">{xpInLevel}/100</div>
          </div>
          <div className="mt-2 text-sm text-neutral-200">Level <span className="font-medium">{levelNow}</span> — Total XP: <span className="font-medium">{state.xp}</span></div>
          <div className="mt-4 flex flex-wrap gap-2">
            <RippleBtn onClick={()=>{incStat("exercises_completed",1); awardXP(10);}}>+10 XP (Latihan)</RippleBtn>
            <RippleBtn onClick={()=>{incStat("bugs_fixed",1); awardXP(5);}}>+5 XP (Tugas Selesai)</RippleBtn>
            <RippleBtn onClick={()=>{incStat("tests_written",3); awardXP(15);}}>+15 XP (Update Info)</RippleBtn>
            <RippleBtn onClick={()=>{incStat("frameworks_tried",1); awardXP(10);}}>+10 XP (Coba Materi baru)</RippleBtn>
            <RippleBtn onClick={()=>logStudy(25,2)}>+ Log Belajar 25m (+2 XP)</RippleBtn>
            <RippleBtn onClick={()=>logStudy(50,5)}>+ Log Belajar 50m (+5 XP)</RippleBtn>
            <RippleBtn onClick={logBreak}>+ Log Istirahat 5m</RippleBtn>
          </div>
        </section>
        ...
        <section className="mb-6 rounded-3xl p-5 bg-neutral-900/90 border border-neutral-700">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {quests.map((q,i)=>(
              <div key={q.id} className={"rounded-2xl border p-4 "+(q.done?"border-emerald-400":"border-neutral-700")}>
        ...
        <section className="mb-6 rounded-3xl p-5 bg-neutral-900/90 border border-neutral-700">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {/** pakai state.badgesUnlocked untuk warna */}
            {(state.badgesUnlocked||[]).length===0 && <div className="text-neutral-300">Belum ada badge — gaskeun ☺️</div>}
            {(state.badgesUnlocked||[]).map(k=>(
              <div key={k} className="rounded-2xl border border-emerald-400 p-4 text-sm">🏅 {k}</div>
            ))}
          </div>
        </section>
        ...
        <section className="mb-6 rounded-3xl p-5 bg-neutral-900/90 border border-neutral-700">
          ...
          <div className="relative h-2 w-full overflow-hidden rounded-full bg-neutral-700 mb-4">
          ...
          <div className="space-y-3">
            {COURSE_SECTIONS.map((sec,si)=>{
              ...
              return (
                <div key={si} className="rounded-2xl border border-neutral-700">
                  <button className="w-full flex items-center justify-between px-4 py-3" onClick={()=>setJsOpen(p=>({...p,[si]:!open}))}>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className={"size-6 rounded-full grid place-items-center "+(open?"bg-neutral-200 text-black":"bg-neutral-800")}>{open?"−":"+"}</div>
                    </div>
                  </button>
                  {open && (
                    <div className="px-4 pb-3">
                      <ul className="space-y-1">
                        {sec.lectures.map((t,li)=>{
                          ...
                          return (
                            <li key={`js-${si}-${li}`} className="flex items-center justify-between rounded-xl px-3 py-2 hover:bg-neutral-800">
                              <div className="flex items-center gap-3">
        ...
        {/* Kurikulum Kustom (CRUD + DnD) */}
        <section className="mb-6 rounded-3xl p-5 bg-neutral-900/90 border border-neutral-700">
          <h2 className="text-lg font-semibold mb-2">{userCourse.title}</h2>
          <div className="flex gap-2 mb-3">
            <input value={userCourse.title} onChange={e=>setState(s=>({...s,userCourse:{...s.userCourse,title:e.target.value}}))} className="flex-1 rounded-xl border border-neutral-700 bg-neutral-900 px-3 py-2"/>
          </div>
          <div className="flex gap-2 mb-3">
            <input value={newSectionTitle} onChange={e=>setNewSectionTitle(e.target.value)} placeholder="Tambah Section..." className="flex-1 rounded-xl border border-neutral-700 bg-neutral-900 px-3 py-2"/>
            <button onClick={()=>{addSection(newSectionTitle); setNewSectionTitle("");}} className="rounded-xl bg-neutral-800 px-4 py-2">Tambah Section</button>
          </div>
          <DragDropContext onDragEnd={onUcDragEnd}>
            <div className="space-y-3">
              {(userCourse.sections||[]).map((sec,si)=>{
                ...
                return (
                  <div key={`uc-sec-${si}`} className="rounded-2xl border border-neutral-700">
                    <div className="flex items-center justify-between px-4 py-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={()=>deleteSection(si)} className="text-xs rounded-xl border border-neutral-700 px-2 py-1 hover:bg-neutral-800">Hapus Section</button>
                        <div className={"size-6 rounded-full grid place-items-center "+(open?"bg-neutral-200 text-black":"bg-neutral-800")}>{open?"−":"+"}</div>
                      </div>
                    </div>
                    {open && (
                      <div className="px-4 pb-3">
                        <div className="flex gap-2 mb-2">
                          <input value={newLectureTitle} onChange={e=>setNewLectureTitle(e.target.value)} placeholder="Tambah lecture..." className="flex-1 rounded-xl border border-neutral-700 bg-neutral-900 px-3 py-2"/>
                          <button onClick={()=>{addLecture(si,newLectureTitle); setNewLectureTitle("");}} className="rounded-xl border border-neutral-700 px-3 py-2 hover:bg-neutral-800">Tambah</button>
                        </div>
                                ...
                                <Draggable key={`uc-${si}-${li}`} draggableId={`uc-${si}-${li}`} index={li}>
                                  {(drag)=>(
                                    <li ref={drag.innerRef} {...drag.draggableProps} {...drag.dragHandleProps}
                                      className="flex items-center justify-between rounded-xl px-3 py-2 hover:bg-neutral-800">
                                      <div className="flex items-center gap-3">
                                      ...
                                      <div className="flex items-center gap-2">
                                        <div className="text-[10px] opacity-70">+{LECTURE_XP} XP</div>
                                        <button onClick={()=>deleteUcLecture(si,li)} className="text-[10px] rounded-xl border border-neutral-700 px-2 py-1 hover:bg-neutral-800">Hapus</button>
                                      </div>
                                    </li>
                                  )}
                                </Draggable>
        ...
        <section className="mb-6 rounded-3xl p-5 bg-neutral-900/90 border border-neutral-700">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
            <input value={capTitle} onChange={e=>setCapTitle(e.target.value)} placeholder="Judul proyek..." className="rounded-xl border border-neutral-700 bg-neutral-900 px-3 py-2"/>
            <input type="number" value={capXp} onChange={e=>setCapXp(e.target.value)} className="rounded-xl border border-neutral-700 bg-neutral-900 px-3 py-2"/>
            <input value={capTopics} onChange={e=>setCapTopics(e.target.value)} placeholder="Topik (pisah koma) ..." className="rounded-xl border border-neutral-700 bg-neutral-900 px-3 py-2"/>
          </div>
          <div className="mb-4"><button onClick={()=>{addCapstone(capTitle,capXp,capTopics); setCapTitle(""); setCapXp(150); setCapTopics("");}} className="rounded-xl bg-neutral-800 px-4 py-2">Tambah Proyek</button></div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {(state.userCapstones||[]).map(c=>(
              <div key={c.id} className="rounded-2xl border border-neutral-700 p-4">
                ...
                <div className="flex gap-2">
                  <button onClick={()=>claimCapstone(c.xp)} className="rounded-xl bg-neutral-800 px-3 py-1 text-sm">Claim XP</button>
                  <button onClick={()=>deleteCapstone(c.id)} className="rounded-xl border border-neutral-700 px-3 py-1 text-sm hover:bg-neutral-800">Hapus</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="mb-6 rounded-3xl p-5 bg-neutral-900/90 border border-neutral-700">
          <div className="mb-3 flex items-center justify-between">
            ...
            <div className="flex gap-2"><button onClick={shufflePeers} className="rounded-xl border border-neutral-700 px-3 py-1 text-sm hover:bg-neutral-800">Acak Skor</button></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {sortedPeers.map((p,i)=>(
              <div key={p.id} className={"flex items-center justify-between rounded-2xl border p-3 "+(p.id==="me"?"border-emerald-400":"border-neutral-700")}>
                <div className="flex items-center gap-3">
                  <div className="size-8 rounded-full grid place-items-center text-xs bg-neutral-800">{i+1}</div>
        ...
        {/* Weekly Summary (panel + modal) */}
        <section className="mb-6 rounded-3xl p-5 bg-neutral-900/90 border border-neutral-700">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold">Weekly Summary</h2>
            <button onClick={()=>setOpenWeekly(true)} className="rounded-xl border border-neutral-700 px-3 py-1 text-sm hover:bg-neutral-800">Lihat Ringkasan</button>
          </div>
        ...
        <section className="mb-6 rounded-3xl p-5 bg-neutral-900/90 border border-neutral-700">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button onClick={exportProgress} className="rounded-2xl border border-neutral-700 px-4 py-2 text-sm hover:bg-neutral-800">Export JSON</button>
      ...
      {/* Weekly Summary Modal (simple, bright) */}
  ...
  return (
    <Modal open={isOpen} onClose={onClose} title="📊 Ringkasan Mingguan" tone="bright">
      <div className="space-y-3">
          ...
          <div className="flex items-end gap-3">
            {series.map((d,i)=>(
              <div key={d.key} className="flex flex-col items-center w-10">
                <div className="w-7 rounded-md" style={{
                  height: Math.max(6, Math.round((d.xp/max)*100))+"px",
                  background: "linear-gradient(#10b981,#22d3ee)"
                }}/>
        ...
        <div className="rounded-xl p-3 border border-neutral-200 bg-white flex items-center gap-3 text-neutral-900">
...
const Pill=({label,value,sub})=>(
  <div className="rounded-xl border border-neutral-200 bg-white text-neutral-900 px-3 py-2">
